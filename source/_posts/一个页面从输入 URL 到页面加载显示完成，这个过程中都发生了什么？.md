...
```
* 本地域名服务器会向根域名服务器发送查询报文。
* 如果根域名服务器也不存在该域名，本地域名服务器会向 .com 顶级域名服务器发送查询报文。
* 如果 .com 顶级域名服务器也不存在该域名，本地域名服务器会向 google.com 顶级域名服务器发送查询报文。得到主机 www 的记录，存入自身缓存并返回给客户端。
```
...
##### 域名级别
	...
	google.com 是二级域名（正常经常忽略 .com，google.com 称为一级域名）
	.com 是一级域名（顶级域名）
...
浏览器根据 `IP` 地址向服务器发起 `TCP` 连接，与浏览器建立 `TCP` 三次握手：
	...
	1. 主机向服务器发送一个建立连接的请求（我要连接你了，可以吗）。
	2. 服务器接到请求后发送同意连接的信号（嗯我准备好了，连接我吧）。
	3. 主机接到同意连接的信号后，再次向服务器发送了确认信号（那我连接你咯），自此，主机与服务器两者建立了连接。
